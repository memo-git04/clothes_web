<?php

namespace App\Http\Controllers\Admin;

use App\Http\Controllers\Controller;
use App\Models\Order;
use App\Models\ProductVariant;
use App\Models\User;
use Carbon\Carbon;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Auth;

class DashboardController extends Controller
{
    public function index()
    {
        $now = Carbon::now();

        // Thống kê nhanh
        $totalProducts = ProductVariant::count();

        $monthlyRevenue = Order::whereYear('orders.created_at', $now->year)
            ->whereMonth('orders.created_at', $now->month)
            ->join('order_statuses', 'orders.status_id', '=', 'order_statuses.id')
            ->where('order_statuses.status_name', '!=', 'Hủy')
            ->sum('orders.final_amount');

        $newCustomers = User::whereMonth('created_at', $now->month)
            ->whereYear('created_at', $now->year)
            ->count();

        $lowStockCount = ProductVariant::where('stock_quantity', '<=', 10)->count();

        // Sản phẩm bán chạy tháng này
        $topProducts = Order::selectRaw('products.product_name, SUM(order_items.quantity) as total_sold')
            ->join('order_items', 'orders.id', '=', 'order_items.order_id')
            ->join('product_variants', 'order_items.product_variant_id', '=', 'product_variants.id')
            ->join('products', 'product_variants.product_id', '=', 'products.id')
            ->join('order_statuses', 'orders.status_id', '=', 'order_statuses.id')
            ->whereYear('orders.created_at', $now->year)
            ->whereMonth('orders.created_at', $now->month)
            ->where('order_statuses.status_name', '!=', 'Hủy')
            ->groupBy('products.id', 'products.product_name')
            ->orderByDesc('total_sold')
            ->limit(5)
            ->get();

        // Sản phẩm sắp hết hàng
        $lowStockProducts = ProductVariant::with(['product', 'color', 'size'])
            ->where('stock_quantity', '<=', 10)
            ->orderBy('stock_quantity', 'asc')
            ->limit(10)
            ->get();

        // Doanh thu 6 tháng gần nhất
        $revenueData = [];
        $revenueLabels = [];

        for ($i = 5; $i >= 0; $i--) {
            $date = $now->copy()->subMonths($i);
            $revenueLabels[] = $date->format('M Y');

            $revenue = Order::whereYear('orders.created_at', $date->year)
                ->whereMonth('orders.created_at', $date->month)
                ->join('order_statuses', 'orders.status_id', '=', 'order_statuses.id')
                ->where('order_statuses.status_name', '!=', 'Hủy')
                ->sum('orders.final_amount');

            $revenueData[] = $revenue ?? 0;
        }

        // Đơn hàng gần đây (Chờ duyệt lên đầu)
        $recentOrders = Order::with(['user', 'status'])
            ->join('order_statuses', 'orders.status_id', '=', 'order_statuses.id')
            ->select('orders.*', 'order_statuses.status_name')
            ->orderByRaw("FIELD(order_statuses.status_name, 'Chờ xác nhận', 'Chờ lấy hàng', 'Đang lấy hàng', 'Đang vận chuyển', 'Giao thành công', 'Hủy')")
            ->orderBy('orders.created_at', 'desc')
            ->limit(10)
            ->get();

        return view('admin.modules.report.index', compact(
            'totalProducts', 'monthlyRevenue', 'newCustomers', 'lowStockCount',
            'topProducts', 'lowStockProducts', 'revenueData', 'revenueLabels', 'recentOrders'
        ));
    }

}
